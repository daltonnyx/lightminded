...
class Basket extends ComponentBase
{
    ...
    public function onCompleteOrder() {
        ...
        $order->save();
        $order = \Jiri\JKShop\Models\Order::find($order->id);
    ...
    public function getSessionBasket() {
        ...
        if (isset($basket["coupon_code"])) {
            ...
            $couponIsValid = false;
            $coupon = \Jiri\JKShop\Models\Coupon::where("code", $basket["coupon_code"])->first();
            $outWrongCode = 0;
            if ($coupon != null) {
        ...
        $basket["payment_method_options"] = \Jiri\JKShop\Models\PaymentGateway::where("active",1)->lists("gateway_title","id");
    ...
    public function onAddToBasket() {
        $basket = $this->getSessionBasket();
        $id = input('id');
        $product = \Jiri\JKShop\Models\Product::where("active",1)->where("available_for_order",1)->find($id);
            ...
            if (array_key_exists($fullKeyProduct, $basket["products"])) {
                ...
                if (input('options')!=null) {
                    foreach (input('options') as $key => $value) {
                        ...
                        $property = \Jiri\JKShop\Models\Property::find($key);
                        if ($property!=null) {
